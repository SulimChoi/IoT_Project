...
double game_dif_setting(int sd, double s1, double s2, double s3, double s4);
...
int main(){
	...
	double setting; // 난이도를 조작하기 위한 변수 
	char sc[20]; // 점수 형변환을 위한 변수 
	char text[100], cpy[] = "SCORE: " ; //clcd 점수 출력문 
	struct timeval starttime ={}; // 라운드 진행 시간을 재기 위한 변수
  ...
  // 초기 설정 
	...
	setting = game_dif_setting(tmp, 1.7, 1.4, 1.2, 1.1);
	...
	for (i = 0; i < 4; i++){
		...
		while(1){
			...
			while(timer < ((int)(setting*10) - (i + 1))){
      ...
      // 라운드 진행 20초 넘길 시 라운드 종료 
			gettimeofday(&endtime, NULL);
			if(20 < (double)(endtime.tv_sec + endtime.tv_usec / 1000000.0 - starttime.tv_sec - starttime.tv_usec / 1000000.0)){
				break;
			}
		}
    ...
    // 다음 라운드 시작 준비 
		if(i != 3){
			close(dot_d);
			close(tactswFd);
			clcd_input("Conglatuation!");
			sleep(3);
			clcd_input("Next Round Start");
			sleep(3);
			dot_d = open(dot, O_RDWR);
...
// 라운드 지속 시간 및 속도 조절 함수
double game_dif_setting(int sd, double s1, double s2, double s3, double s4){
	double tmp2;
	switch(sd){
		case 4: tmp2 = s1; break;
		case 5: tmp2 = s2; break;
		case 6: tmp2 = s3; break;
		case 7: tmp2 = s4; break;
	}
	return tmp2;
}
